...
inline void set_proj_type(const std::string &proj_type, const float focal_length = 1)
{
    detail::_interpreter::get();
    ...
    PyObject* pystr = PyString_FromString(proj_type.c_str());
    PyObject* args = PyTuple_New(1);
    PyTuple_SetItem(args, 0, pystr);

    PyObject* pyfloat = PyFloat_FromDouble(focal_length);
    PyObject* args2 = PyTuple_New(1);
    PyTuple_SetItem(args2, 0, pyfloat);
    ...
    PyObject *projType = PyObject_GetAttrString(ax, "set_proj_type");
    if (!projType) throw std::runtime_error("Attribute set_proj_type not found.");
    // Py_INCREF(projType);

    PyObject *res = PyObject_Call(projType, args, kwargs);
    if (!res) throw std::runtime_error("Call to set_proj_type() failed.");
    Py_DECREF(projType);
